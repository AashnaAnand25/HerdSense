...
import { PENS } from "@/data/syntheticData";
import { motion } from "framer-motion";
import { AlertTriangle, Activity, Thermometer, ChevronRight } from "lucide-react";
...
export default function LiveFeed() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setElapsed(e => {
        if (e >= 100) { setIsPlaying(false); return 100; }
        return e + 1;
      });
          ...
          {PENS.map((pen, i) => (
            <motion.div
              ...
              className={`card-glass rounded-xl overflow-hidden transition-all ${
                pen.status === "alert" ? "card-danger" : pen.status === "warning" ? "border border-warning/30" : ""
              }`}
              ...
              <div className="p-4 space-y-3">
                  ...
                  <div className="h-1.5 rounded-full bg-field-600 overflow-hidden">
                    <motion.div
                      ...
                      style={{
                        ...
                        background: pen.avgActivity < 40 ? "hsl(3 79% 57%)" : pen.avgActivity < 60 ? "hsl(37 91% 55%)" : "hsl(88 100% 62%)",
                      }}
                      initial={{ width: 0 }}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1 text-xs font-mono">
                    <Thermometer size={11} className={pen.temperature > 39.5 ? "text-warning" : "text-muted-foreground"} />
                    <span className={pen.temperature > 39.5 ? "text-warning" : "text-muted-foreground"}>
        ...
        <div className="card-glass rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs font-mono text-muted-foreground mt-0.5">Scrub through a simulated day to see how alerts appear over time</p>
            </div>
            <button
              onClick={() => { setIsPlaying(!isPlaying); if (!isPlaying) { setElapsed(0); setScrubber(0); } }}
              className={`px-4 py-2 rounded-lg font-mono text-xs font-semibold transition-all ${
                isPlaying ? "bg-warning/20 text-warning border border-warning/30" : "bg-primary text-primary-foreground shadow-glow-lime"
              }`}
            >
          ...
          <div className="flex justify-between text-[10px] font-mono text-muted-foreground">
            <span>00:00</span>
            <span className="text-primary">{String(Math.floor(scrubber * 0.24)).padStart(2, "0")}:{String(Math.round((scrubber * 0.24 % 1) * 60)).padStart(2, "0")}</span>
            <span>24:00</span>
          </div>
